...
#         from .utils import build_user_section_map, RESPONSABLE_VARIANTS, get_appellation_map
#         user_section_map_full = build_user_section_map()
#         appellation_map = get_appellation_map()
#         user_section_map = {
...
#                     #"name": user.name,
#                     "name": appellation_map.get(user.badgenumber) or user.name,  # ✅
...
#                 etat = 'multiples_pointages'
#                 h_rect_entree = h_rect_sortie = None
#             elif type_anomalie == 'retard_sortie':             
#                 etat = 'retard_sortie'
#                 h_rect_entree, h_rect_sortie = h_entree, h_sortie
#             # elif type_anomalie == 'entree_trop_tot':      # ← AJOUTÉ
#             #     etat = 'entree_trop_tot'
#             #     h_rect_entree, h_rect_sortie = h_entree, h_sortie
#             elif type_anomalie == 'pas_entree' or h_entree is None:
#                 etat = 'pas_entree'
...
#             #     continue
#             # Les anomalies OK sont supprimées SAUF si correction manuelle
#             # key = (userid, date_jour)
#             # if etat == 'ok':
#             #     if key in existing:
#             #         existing_obj = existing[key]
#             #         # Conserver si correction manuelle (heures rectifiées présentes)
#             #         if existing_obj.heure_rectifiee_entree or existing_obj.heure_rectifiee_sortie:
#             #             pass  # ne pas supprimer
#             #         else:
#             #             to_delete_ids.append(existing_obj.pk)
#             #     continue
#             key = (userid, date_jour)
#             if etat == 'ok':
#                 if key in existing:
#                     existing_obj = existing[key]
#                     # Supprimer si entree_trop_tot (ancien état obsolète)
#                     if existing_obj.etat == 'entree_trop_tot':
#                         to_delete_ids.append(existing_obj.pk)
#                     # Conserver si correction manuelle (heures rectifiées présentes)
#                     elif existing_obj.heure_rectifiee_entree or existing_obj.heure_rectifiee_sortie:
#                         pass  # ne pas supprimer
#                     else:
#                         to_delete_ids.append(existing_obj.pk)
#                 continue
...
#         entree = entree.replace(second=0, microsecond=0)
#         sortie = sortie.replace(second=0, microsecond=0)
...
#         def _ht(duree):
#             return duree if duree < 6 else duree - 0.5

#         if est_samedi:
#             if type_evenement in ('X', 'HA', 'OS', 'PS'):
#                 hs = duree_heures if duree_heures < 6 else duree_heures - 0.5
#             else:
#                 hs = 0.0
#             ht = _ht(duree_heures)
...
#         if type_evenement == 'X':
#             hs = duree_heures - 8 if duree_heures < 8 else duree_heures - 0.5 - 8
#             ht = _ht(duree_heures)
#         else:
#             # Event != 'X' : si durée < 8h → HT = 8h (complété par l'événement)
#             #                si durée >= 8h → calcul normal
#             if type_evenement == 'HA':
#                 hs = duree_heures - 7 if duree_heures < 8 else duree_heures - 0.5 - 7
#             elif type_evenement in ('OS', 'PS'):
#                 hs = duree_heures - 8.5
#             else:
#                 hs = 0.0

#             ht = 8.0 if duree_heures < 8 else _ht(duree_heures)
...
#                         and anomalie.heure_rectifiee_sortie):
#                     entree = anomalie.heure_rectifiee_entree
#                     # Pour retard_sortie non modifié manuellement : utiliser l'heure réelle (prévue)
#                     if (anomalie.etat == 'retard_sortie'
#                             and anomalie.heure_rectifiee_sortie == anomalie.heure_brute_sortie
#                             and anomalie.heure_reelle_sortie):
#                         sortie = anomalie.heure_reelle_sortie
#                     else:
#                         sortie = anomalie.heure_rectifiee_sortie
...
#                         pts, h_e, h_s, seuil_minutes=30
#                     )
#                     if he and hs and type_a is None:
#                         # Cas normal : appliquer la même correction que la détection
#                         # Entrée : si brute <= prévue → prévue, sinon → brute (retard)
#                         entree = h_e if he <= h_e else he
#                         sortie = hs
#                     elif he and hs:
#                         entree, sortie = he, hs
...
# class AbsencesMoisAPIView(APIView):
#     """
#     Calcul des absences par type pour un mois donné.
#     - CM, HA, TCP : compteurs en jours (0 ou 1 par jour)
#     - CP, PS, ANO, MP, RM, AMP, EF, HP, RC : valeurs en heures
#     GET /api/presence/absences-mois/?annee=2025&mois=8&section=COUPE&page=1&page_size=50&q=
#     """
#     permission_classes = [AllowAny]

#     # ── Clés absence ──────────────────────────────────────────────────────────
#     ABS_KEYS = ['cm', 'cp', 'ps', 'ano', 'mp', 'rm', 'amp', 'ef', 'hp', 'rc', 'ha', 'tcp']
#     # cm, ha, tcp = jours (int)   |   cp, ps, ano, mp, rm, amp, ef, hp, rc = heures (float)
#     JOURS_KEYS = {'cm', 'ha', 'tcp'}

#     @staticmethod
#     def _calc_absences(entree, sortie, event, est_samedi, est_dimanche):
#         """
#         Calcule les valeurs d'absence pour un jour.
#         Retourne un dict {clé: valeur}.

#         Formules :
#         ┌─────────────────────────────────────────────────────────────────────┐
#         │ Jour normal (lun–ven) :                                             │
#         │   HT = durée < 6h ? durée : durée − 0.5                            │
#         │   CM  = event="CM" → 1 (jour)                                      │
#         │   HA  = event="HA" → 1 (jour)                                      │
#         │   CP/PS/ANO/MP/RM/AMP/EF/HP/RC :                                  │
#         │      event=code ? (pas présence → 8h) | (HT≥8 → 0) | (8−HT) : 0  │
#         │   TCP = event null ou CM → 0                                        │
#         │         event PS/ANO/MP sans présence → 0                           │
#         │         sinon → 1 (jour)                                            │
#         ├─────────────────────────────────────────────────────────────────────┤
#         │ Samedi / Dimanche :                                                 │
#         │   PS, ANO, MP → 0 (pas d'absence samedi/dimanche pour ces codes)   │
#         │   Reste idem jour normal                                            │
#         └─────────────────────────────────────────────────────────────────────┘
#         """
...
#         #     ref = d_cls.today()
#         #     duree = (
#         #         dt_cls.combine(ref, sortie) - dt_cls.combine(ref, entree)
#         #     ).total_seconds() / 3600.
#         # ── Durée et HT ───────────────────────────────────────────────────────
#         if entree and sortie:
#             entree = entree.replace(second=0, microsecond=0)
...
#             ht = max(0.0, duree if duree < 6 else duree - 0.5)
#             pres = True
#         else:
#             ht = 0.0
#             pres = False

#         # ── Absence standard (en heures) ─────────────────────────────────────
#         def abs_h(code, seuil=8.0):
#             if event != code:
#                 return 0.0
#             if not pres:          # pas d'entrée ou pas de sortie → journée complète
#                 return seuil
#             return max(0.0, seuil - ht)

#         # ── CM et HA : compteurs de jours ─────────────────────────────────────
#         cm = 1 if event == 'CM' else 0
#         ha = 1 if event == 'HA' else 0

#         # ── CP, RM, AMP, EF, HP, RC : communs à tous les jours ───────────────
#         cp  = abs_h('CP')
#         rm  = abs_h('RM')
#         amp = abs_h('AMP')
#         ef  = abs_h('EF')
...
#         # ── PS, ANO, MP : uniquement jours normaux (0 samedi/dimanche) ────────
#         if est_samedi or est_dimanche:
#             ps  = 0.0
#             ano = 0.0
#             mp  = 0.0
#         else:
...
#         return {
#             'cm': cm, 'cp': round(cp, 2), 'ps': round(ps, 2),
#             'ano': round(ano, 2), 'mp': round(mp, 2), 'rm': round(rm, 2),
#             'amp': round(amp, 2), 'ef': round(ef, 2), 'hp': round(hp, 2),
...
#         # from .utils import (
#         #     build_user_section_map, analyser_pointages_jour,
#         #     get_horaire_pour_date, get_active_userinfo_queryset,
#         # )
#         from .utils import (
#             build_user_section_map, analyser_pointages_jour,
#             get_horaire_pour_date, get_active_userinfo_queryset,
...
#         # Numéros de semaine présents dans la période
#         semaine_nums = sorted({
#             d.code_date[0] for d in dates_mois if d.code_date
#         })
...
#         evenements_map = {
#             (ev['userid'], ev['date']): ev['type_evenement']
#             for ev in Evenement.objects.filter(
#                 userid__in=user_ids, date__in=dates_liste
#             ).values('userid', 'date', 'type_evenement')
#         }
...
#             # Accumulateurs
#             par_semaine = {s: {k: 0.0 for k in ABS_KEYS} for s in semaine_nums}
#             total_abs   = {k: 0.0 for k in ABS_KEYS}
#             details_jours = []

#             for date_obj in dates_mois:
#                 date_jour  = date_obj.date
#                 semaine    = date_obj.code_date[0] if date_obj.code_date else semaine_nums[0]
#                 key        = (user.userid, date_jour)
#                 event      = evenements_map.get(key, 'X')

#                 # Résoudre entrée / sortie
#                 entree = sortie = None
#                 anomalie = anomalies_map.get(key)
#                 if anomalie and anomalie.heure_rectifiee_entree and anomalie.heure_rectifiee_sortie:
#                     entree = anomalie.heure_rectifiee_entree
...
#                 est_samedi   = date_jour.weekday() == 5
#                 est_dimanche = date_jour.weekday() == 6

#                 # ── Calcul des absences du jour ───────────────────────────────
#                 abs_j = self._calc_absences(entree, sortie, event, est_samedi, est_dimanche)

#                 for k in ABS_KEYS:
#                     v = abs_j.get(k, 0)
#                     par_semaine[semaine][k] = round(par_semaine[semaine].get(k, 0) + v, 2)
#                     total_abs[k]           = round(total_abs.get(k, 0) + v, 2)
...
#                     'event':     event,
#                     'ht':        abs_j['ht'],
#                     **{k: abs_j.get(k, 0) for k in ABS_KEYS},
#                 })
...
#                 'par_semaine': par_semaine,
#                 'details_jours': details_jours,
#                 **{f'total_{k}': total_abs[k] for k in ABS_KEYS},
#             })
...
#         return Response({
#             'periode': {
#                 'mois': mois_fr[mois - 1], 'annee': annee,
#                 'du': f"21 {mois_fr[mois_precedent - 1]}",
#                 'au': f"20 {mois_fr[mois - 1]}",
#             },
#             'semaines': semaine_nums,
...
#                 'total_employees': 0, 'total_pages': 0,
...
# # class VerificationPresencesAPIView(APIView):
# #     permission_classes = [AllowAny]
...
# #             mois      = int(request.query_params.get('mois',  date.today().month))
# #             page      = max(1, int(request.query_params.get('page', 1)))
# #             page_size = min(200, max(1, int(request.query_params.get('page_size', 100))))
# #         except ValueError:
# #             return Response({"error": "Paramètres invalides"}, status=400)
...
# #         from .utils import (build_user_section_map, get_active_userinfo_queryset,
# #                             get_appellation_map, get_horaire_pour_date, analyser_pointages_jour)
...
# #         all_userids = list(users_qs.values_list('userid', flat=True))

# #         # ── Pré-chargements ──────────────────────────────────────────────────
# #         # Pointages bruts
# #         pointages_map = {}
# #         for p in (CheckInOut.objects
# #                   .filter(checktime__date__in=dates_liste, user_id__in=all_userids)
# #                   .values('user_id', 'checktime', 'checktype')):
# #             pointages_map.setdefault((p['user_id'], p['checktime'].date()), []).append(p)
...
# #             (ev['userid'], ev['date']): ev['type_evenement']
# #             for ev in Evenement.objects.filter(
# #                 userid__in=all_userids, date__in=dates_liste
# #             ).values('userid', 'date', 'type_evenement')
# #         }

# #         # Toutes les anomalies (pas seulement ok) pour détecter les non corrigées
# #         anomalies_map = {
# #             (a.userid, a.date): a
# #             for a in Anomalie.objects.filter(
# #                 date__in=dates_liste, userid__in=all_userids
# #             )
# #         }
...
# #             section_emp = user_section_map.get(user.userid, 'ADMINISTRATION')
# #             horaire     = horaires_dict.get(section_emp, horaire_default)
# #             nom         = appellation_map.get(user.badgenumber) or user.name
...
# #                 # ── Résoudre entree/sortie depuis les RECTIFIÉES ─────────────
# #                 entree = sortie = None

# #                 if anomalie:
# #                     # Anomalie présente : on prend toujours les rectifiées
# #                     entree = anomalie.heure_rectifiee_entree
# #                     sortie = anomalie.heure_rectifiee_sortie
# #                 elif key in pointages_map and horaire:
# #                     # Pas d'anomalie → analyser les bruts (cas normal sans anomalie)
# #                     h_e, h_s = get_horaire_pour_date(
# #                         horaire, date_jour,
# #                         date_obj.est_jour_paiement,
# #                         section=section_emp,
...
# #                 # 1. Pas d'entrée ET pas de sortie, mais événement non-X
# #                 if entree is None and sortie is None and event == 'HA':
# #                     type_erreur = 'absence_avec_evenement'

# #                 # 2. Entrée == Sortie
# #                 elif entree is not None and sortie is not None and entree == sortie:
# #                     type_erreur = 'entree_egale_sortie'

# #                 # 3. Sortie avant entrée
# #                 elif entree is not None and sortie is not None and sortie < entree:
# #                     type_erreur = 'sortie_avant_entree'

# #                 # 4. Entrée ET sortie présentes, mais pas d'événement (X ou absent)
# #                 elif entree is not None and sortie is not None and event == 'X':
# #                     # Uniquement si c'est une anomalie corrigée manuellement
# #                     # (présence normale avec X = travail normal, pas une erreur)
# #                     if anomalie and anomalie.etat == 'ok' \
# #                             and anomalie.heure_rectifiee_entree \
# #                             and anomalie.heure_rectifiee_sortie \
# #                             and not key in pointages_map:
# #                         type_erreur = 'presence_sans_evenement'
...
# #                         'date':        date_jour.isoformat(),
# #                         'code_date':   date_obj.code_date,
# #                         'entree':      str(entree)[:5] if entree else None,
# #                         'sortie':      str(sortie)[:5] if sortie else None,
# #                         'evenement':   event,
# #                         'type_erreur': type_erreur,
# #                         'anomalie_etat': anomalie.etat if anomalie else None,
# #                     })
...
# #         page        = min(page, total_pages)
# #         erreurs_page = erreurs[(page - 1) * page_size: page * page_size]

# #         from collections import Counter
# #         counts = Counter(e['type_erreur'] for e in erreurs)

# #         return Response({
# #             'periode': {'annee': annee, 'mois': mois},
# #             'statistiques': {
# #                 'total':                   total,
# #                 'absence_avec_evenement':  counts.get('absence_avec_evenement', 0),
# #                 'entree_egale_sortie':     counts.get('entree_egale_sortie', 0),
# #                 'sortie_avant_entree':     counts.get('sortie_avant_entree', 0),
# #                 'presence_sans_evenement': counts.get('presence_sans_evenement', 0),
# #             },
# #             'pagination': {
# #                 'page':         page,
# #                 'page_size':    page_size,
...
#         cache_key = f"verif_presences_{annee}_{mois}"

#         erreurs_all = None if force_refresh else cache.get(cache_key)

#         if erreurs_all is None:
#             t1 = time.time()
#             erreurs_all = self._calculer_erreurs(annee, mois)
#             logger.warning(
#                 f"[VERIF] CALCUL COMPLET: {time.time() - t1:.2f}s "
#                 f"({len(erreurs_all)} erreurs trouvées)"
#             )
#             cache.set(cache_key, erreurs_all, self.CACHE_TTL)
#         else:
#             logger.warning(f"[VERIF] CACHE HIT ({len(erreurs_all)} erreurs)")

#         erreurs = erreurs_all
#         if section_filter:
#             erreurs = [e for e in erreurs if e['section'].upper() == section_filter]
#         if q:
#             ql = q.lower()
#             erreurs = [
#                 e for e in erreurs
#                 if ql in e['nom'].lower() or ql in e['badgenumber'].lower()
#             ]
...
#         logger.warning(f"[VERIF] TOTAL requête: {time.time() - t0:.2f}s")
...
#     def _calculer_erreurs(self, annee, mois):
#         """Calcul complet, non filtré, sur TOUS les employés actifs (mis en cache par l'appelant)."""
#         from .utils import (build_user_section_map, get_active_userinfo_queryset,
#                              get_appellation_map, decimal_to_time, analyser_pointages_jour)
#         from .models import HoraireException
...
#         users_list  = list(get_active_userinfo_queryset().order_by('badgenumber'))
#         all_userids = [u.userid for u in users_list]

#         # ── Pointages bruts : UNE requête pour toute la période ──────────────
#         pointages_map = {}
#         for p in (CheckInOut.objects
#                   .filter(checktime__date__in=dates_liste, user_id__in=all_userids)
#                   .values('user_id', 'checktime', 'checktype')):
...
#         # ── Anomalies : UNE requête ────────────────────────────────────────────
#         anomalies_map = {
#             (a.userid, a.date): a
#             for a in Anomalie.objects.filter(date__in=dates_liste, userid__in=all_userids)
#         }

#         # ── Horaires de section : UNE requête ─────────────────────────────────
#         horaires_dict   = {h.section: h for h in HoraireSection.objects.all()}
#         horaire_default = horaires_dict.get('ADMINISTRATION')

#         # ── CORRECTIF PRINCIPAL : précharger TOUTES les exceptions d'horaire
#         # de la période en UNE requête, au lieu de 2 requêtes par (user, jour)
#         # comme le faisait get_horaire_pour_date() appelé dans la boucle. ──────
#         exceptions_map = {}
#         for ex in HoraireException.objects.filter(date__in=dates_liste):
#             exceptions_map[(ex.date, ex.section)] = ex
#             if ex.section is None:
...
#         def resoudre_horaire(horaire, date_jour, est_paiement, section):
#             """Version sans requête DB — utilise exceptions_map préchargé."""
#             exception = exceptions_map.get((date_jour, section)) or \
#                         exceptions_map.get((date_jour, None))
#             if exception:
...
#                     sortie = anomalie.heure_rectifiee_sortie
#                 elif key in pointages_map and horaire:
#                     h_e, h_s = resoudre_horaire(
#                         horaire, date_jour, date_obj.est_jour_paiement, section_emp
#                     )
#                     pts = [FP(p) for p in pointages_map[key]]
#                     he, hs, type_a, _ = analyser_pointages_jour(pts, h_e, h_s, seuil_minutes=30)
#                     entree, sortie = he, hs
...
#                             and anomalie.heure_rectifiee_entree \
#                             and anomalie.heure_rectifiee_sortie \
#                             and key not in pointages_map:
#                         type_erreur = 'presence_sans_evenement'

#                 if type_erreur:
#                     erreurs.append({
#                         'userid': user.userid, 'badgenumber': user.badgenumber, 'nom': nom,
#                         'section': section_emp, 'date': date_jour.isoformat(),
#                         'code_date': date_obj.code_date,
#                         'entree': str(entree)[:5] if entree else None,
#                         'sortie': str(sortie)[:5] if sortie else None,
#                         'evenement': event, 'type_erreur': type_erreur,
#                         'anomalie_etat': anomalie.etat if anomalie else None,
#                     })
#         return erreurs
...
class HoraireSectionDetailAPIView(APIView):
    ...
    def put(self, request, section):
        ...
        serializer = HoraireSectionSerializer(horaire, data=request.data, partial=True)
        if serializer.is_valid():
            horaire_maj = serializer.save()
            # ← Répercute automatiquement le nouvel horaire sur les anomalies
            # déjà enregistrées pour cette section (heure "prévue" par défaut).
            nb_maj = self._refresh_anomalies_pour_section(horaire_maj)
            data = serializer.data
            if nb_maj is None:
                data['anomalies_mises_a_jour'] = 0
                data['avertissement'] = (
                    "L'horaire a été enregistré, mais la mise à jour des heures "
                    "par défaut existantes a échoué (voir les logs serveur)."
                )
            else:
                data['anomalies_mises_a_jour'] = nb_maj
            return Response(data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def _refresh_anomalies_pour_section(self, horaire):
        """
        Après modification de l'horaire d'une section, recalcule heure_reelle_entree
        et heure_reelle_sortie (l'heure "prévue" par défaut) sur toutes les anomalies
        déjà enregistrées pour cette section — sans jamais toucher aux heures
        rectifiées (corrections manuelles conservées).

        Version optimisée : toutes les exceptions d'horaire concernées sont
        préchargées en 2 requêtes (au lieu d'interroger la base pour chaque
        anomalie une par une, ce qui rendait l'enregistrement très lent sur
        les sections avec beaucoup d'employés/mois).
        """
        try:
            from datetime import timedelta
            # On ne recalcule que les 2 derniers mois (période de paie en cours
            # + précédente) : recalculer tout l'historique était trop lourd et
            # rendait l'enregistrement de l'horaire très lent.
            date_limite = date.today() - timedelta(days=60)

            anomalies = list(
                Anomalie.objects.filter(
                    section=horaire.section, date__gte=date_limite
                )
            )
            if not anomalies:
                return 0

            dates_concernees = {a.date for a in anomalies}

            dates_map = {
                d.date: d.est_jour_paiement
                for d in Date.objects.filter(date__in=dates_concernees)
            }

            exceptions_section = {
                e.date: e
                for e in HoraireException.objects.filter(
                    section=horaire.section, date__in=dates_concernees
                )
            }
            exceptions_globales = {
                e.date: e
                for e in HoraireException.objects.filter(
                    section__isnull=True, date__in=dates_concernees
                )
            }

            entree_normale = decimal_to_time(horaire.heure_entree)
            sortie_normale = decimal_to_time(horaire.heure_sortie)
            sortie_samedi = decimal_to_time(horaire.sortie_samedi)
            sortie_vendredi_paiement = decimal_to_time(horaire.sortie_vendredi_paiement)
            sortie_samedi_paiement = decimal_to_time(horaire.sortie_samedi_paiement)

            to_update = []
            for anomalie in anomalies:
                d = anomalie.date
                est_samedi = d.weekday() == 5
                est_vendredi = d.weekday() == 4
                est_paiement = dates_map.get(d, False)

                exception = exceptions_section.get(d) or exceptions_globales.get(d)
                if exception:
                    nouvelle_entree = exception.heure_entree or entree_normale
                    nouvelle_sortie = exception.heure_sortie or sortie_normale
                else:
                    nouvelle_entree = entree_normale
                    if est_paiement and est_vendredi:
                        nouvelle_sortie = sortie_vendredi_paiement
                    elif est_paiement and est_samedi:
                        nouvelle_sortie = sortie_samedi_paiement
                    elif est_samedi:
                        nouvelle_sortie = sortie_samedi
                    else:
                        nouvelle_sortie = sortie_normale

                if (
                    anomalie.heure_reelle_entree != nouvelle_entree
                    or anomalie.heure_reelle_sortie != nouvelle_sortie
                ):
                    anomalie.heure_reelle_entree = nouvelle_entree
                    anomalie.heure_reelle_sortie = nouvelle_sortie
                    to_update.append(anomalie)
            ...
            if to_update:
                ...
                logger.info(
                    f"HoraireSection {horaire.section}: "
                    f"{len(to_update)} anomalie(s) rafraîchie(s) après modification de l'horaire"
                )
            return len(to_update)
        except Exception as e:
            # On ne fait plus échouer l'enregistrement de l'horaire à cause
            # d'un souci sur le rafraîchissement des anomalies, mais on log
            # la trace complète pour pouvoir diagnostiquer si ça se reproduit.
            logger.error(f"_refresh_anomalies_pour_section a échoué: {e}", exc_info=True)
            return None
...
class PresenceMoisDetailCalculeeAPIView(APIView):
    ...
    def get(self, request):
        ...
        # 6. Anomalies corrigées pour la page courante
        # On inclut les anomalies "ok" (résolues automatiquement) ET celles où
        # au moins une heure rectifiée a été saisie manuellement (entrée seule
        # ou sortie seule) — sinon une saisie manuelle partielle (ex: entrée
        # sans sortie, sur un jour sans pointage brut) disparaissait de la grille
        # car elle restait à l'état 'pas_sortie'/'pas_entree' et ne matchait pas
        # etat='ok', tout en n'ayant pas non plus de pointages bruts en secours.
        anomalies_qs = Anomalie.objects.filter(
            date__in=dates_liste,
            userid__in=user_ids_page
        ).filter(
            Q(etat='ok') |
            Q(heure_rectifiee_entree__isnull=False) |
            Q(heure_rectifiee_sortie__isnull=False)
        )
        anomalies_map = {}
        for a in anomalies_qs:
            key = (a.userid, str(a.date))
        ...
        for user in users_page:
            ...
            for date_obj in dates_mois:
                ...
                resultat.append({
                    ...
                    "est_jour_paiement": est_jour_paiement,
                    "est_anomalie_corrigee": anomalie_corrigee is not None,
                    "anomalie_etat": anomalie_corrigee.get('etat') if anomalie_corrigee else None,
                    "synchronise_le": str(synchronise_le) if synchronise_le else None,
                    "present": present,
                    "heure_brute_entree": str(heure_brute_entree) if heure_brute_entree else None,
                    "heure_brute_sortie": str(heure_brute_sortie) if heure_brute_sortie else None,
                    ...
                    "est_sorti_en_avance": analyse['est_sorti_en_avance'],
                    "difference_heures": analyse['difference_heures'],
                    "nombre_pointages_bruts": len(pointages_du_jour),
                    "evenement": evenement_data['type'] if evenement_data else 'X',
                    "evenement_commentaire": evenement_data['commentaire'] if evenement_data else "",
                    "has_special_event": bool(evenement_data and evenement_data['type'] != 'X'),
                })
...
class HeuresTravailAPIView(APIView):
    ...
    def get(self, request):
        ...
        # ── Pré-chargements ──────────────────────────────────────────────────
        # Anomalies corrigées (état 'ok') OU corrigées partiellement (au moins
        # une heure rectifiée saisie manuellement) — voir note dans
        # PresenceMoisDetailCalculeeAPIView pour le détail du problème corrigé.
        anomalies_map: dict = {}
        for a in Anomalie.objects.filter(
            date__in=dates_liste, userid__in=user_ids
        ).filter(
            Q(etat='ok') |
            Q(heure_rectifiee_entree__isnull=False) |
            Q(heure_rectifiee_sortie__isnull=False)
...
class IndemniteRepasAPIView(APIView):
    ...
    def get(self, request):
        ...
        # Préchargements
        anomalies_map = {}
        for a in Anomalie.objects.filter(date__in=dates_liste, userid__in=user_ids).filter(
            Q(etat='ok') |
            Q(heure_rectifiee_entree__isnull=False) |
            Q(heure_rectifiee_sortie__isnull=False)
        ):
    ...
    def _update_anomalie(self, request, pk, partial=False):
        ...
        if serializer.is_valid():
            try:
                ...
                if 'lock timeout' in str(e).lower() or 'lock_timeout' in str(e).lower():
                    return Response(
                        {"error": (
                            "Impossible d'enregistrer : un autre utilisateur ou une "
                            "fenêtre pgAdmin ouverte bloque cette donnée en base. "
                            "Fermez/validez (Commit) les requêtes ouvertes dans pgAdmin "
                            "puis réessayez."
                        )},
...
class ModifierHeuresManuellementAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # On conserve systématiquement les heures brutes, on ne les écrase jamais.
            # Les heures rectifiées sont celles saisies par l'utilisateur.
            # lock_timeout : si une autre session (ex: pgAdmin avec une
            # transaction restée ouverte) bloque la ligne, on échoue en 5s
            # avec une erreur claire au lieu de rester bloqué indéfiniment.
            try:
                with transaction.atomic():
                    with connection.cursor() as cursor:
                        cursor.execute("SET LOCAL lock_timeout = '5s'")
